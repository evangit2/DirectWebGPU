...
export function decodeDraw(memory,pointer,length){
 ...
 if(magic!==0x39445244||![1,2,4].includes(kind)||count<1||count>1048576||count%({1:1,2:2,4:3})[kind]||declarationLength<16||declarationLength>520||declarationLength%8||stateCount!==20)throw RangeError('invalid draw header');
 ...
 return{vertex,pixel,kind,count,first,index,base:base|0,max,streams,state,declaration,registers};
}
export class DrawRenderer{
 constructor(device,backend){this.device=device;this.backend=backend;this.cache=new PipelineCache(device,backend.shaders);this.uniforms=[0,1].map(()=>device.createBuffer({size:4608,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}));}
 async draw(packet){
  ...
  // Texture reflection/binding is the next draw extension; never ignore a texture input.
  if(/@group\(0\)|@group\(2\)/.test(entry.shaders.vertex.wgsl+entry.shaders.pixel.wgsl))throw RangeError('textured draw binding not implemented');
  ...
  const groups=[],last=packed[1].length?3:packed[0].length?1:-1;
  for(let group=0;group<=last;group++){const stage=group===1?0:group===3?1:-1;let entries=[];if(stage>=0&&packed[stage].length){d.queue.writeBuffer(this.uniforms[stage],0,packed[stage]);entries=[{binding:0,resource:{buffer:this.uniforms[stage],size:packed[stage].byteLength}}]}groups.push(d.createBindGroup({layout:entry.pipeline.getBindGroupLayout(group),entries}));}
  ...
  pass.setPipeline(entry.pipeline);packet.state.applyDynamic(pass);bindings.forEach((s,i)=>pass.setVertexBuffer(i,s.buffer,s.offset,s.size));groups.forEach((g,i)=>pass.setBindGroup(i,g));
 ...
 dispose(){this.cache.dispose();for(const u of this.uniforms)u.destroy();}
}
