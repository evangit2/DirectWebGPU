...
import {createShaderTranslator,canonicalFloatLimits} from './shaders.js';
...
document.getElementById('run').onclick=async()=>{
 ...
 try{
  ...
  device=await adapter.requestDevice({requiredFeatures:adapter.features.has('texture-compression-bc')?['texture-compression-bc']:[]});const queryMemory=new SharedArrayBuffer(8192),queryPointer=4096,queryBackend={draws:{flush(){}}};Atomics.store(new Int32Array(queryMemory),queryPointer/4,0);const queryEncoder=device.createCommandEncoder();device.queue.submit([queryEncoder.finish()]);issueEventQuery(device,queryBackend,queryMemory,queryPointer);if(Atomics.load(new Int32Array(queryMemory),queryPointer/4)!==0)throw Error('event query completed synchronously');await flushEventQuery(device,queryBackend,queryMemory,queryPointer);if(Atomics.load(new Int32Array(queryMemory),queryPointer/4)!==1)throw Error('event query fence did not complete');report.eventQuery={result:'passed',checks:['Issue marker stayed asynchronous','D3DGETDATA_FLUSH boundary completed submitted WebGPU work','shared completion word signaled']};report.checks.push('asynchronous event-query marker and explicit flush completed on WebGPU');const limitSource=canonicalFloatLimits('@compute @workgroup_size(1) fn main(){let x=340282350000000000000000000000000000000f; let y=-340282350000000000000000000000000000000f;}');const limitInfo=await device.createShaderModule({code:limitSource}).getCompilationInfo();if(limitInfo.messages.some(m=>m.type==='error'))throw Error('f32 limit shader failed: '+limitInfo.messages.map(m=>m.message).join(';'));report.floatLimit={result:'passed',source:limitSource};report.drawPackets=await testDrawPackets(device,tr,vs,ps);report.samplers=await testSamplers(device);report.textures=await testTextures(device);report.geometryBuffers=await testGeometryBuffers(device);const errors=[];device.addEventListener('uncapturederror',e=>errors.push(e.error.message));
