...
export function fixedFunctionPair(declaration,textured,viewportSize=[1,1],textureStages=null,lighting=null,clipTransformed=true,depthEnabled=true,textureDimension=1){
 ...
 const coordMode=Number(stage[6])&0xffff0000;
 ...
 const generatedCube=textureDimension===3&&coordMode!==0;
 if(![1,3].includes(textureDimension))throw Error('unsupported fixed-function texture dimension '+textureDimension);
 if(generatedCube&&![0x10000,0x20000,0x30000].includes(coordMode))throw Error('unsupported cube coordinate generation '+coordMode);
 if(generatedCube&&(coordMode===0x10000||coordMode===0x30000)&&!hasNormal)throw Error('cube normal/reflection coordinates require NORMAL');
 const passthroughReady=hasTexcoord&&(textureDimension===1||texcoordType>=2);
 const useTexture=Boolean(textured&&(generatedCube||passthroughReady));
 ...
 const uvType=textureDimension===3?'vec3<f32>':'vec2<f32>';
 const io='struct Output { @builtin(position) position:vec4<f32>, @location(0) color:vec4<f32>, @location(1) uv:'+uvType+' };';
 ...
 const objectPosition=position==='xyzw'?'position':'vec4<f32>(position,1.0)';
 const cubeCoordinate=coordMode===0x10000
  ? 'normalize((transforms.view*transforms.world*vec4<f32>(vertexAttr,0.0)).xyz)'
  : coordMode===0x20000
   ? '(transforms.view*transforms.world*'+objectPosition+').xyz'
   : coordMode===0x30000
    ? 'reflect(normalize((transforms.view*transforms.world*'+objectPosition+').xyz),normalize((transforms.view*transforms.world*vec4<f32>(vertexAttr,0.0)).xyz))'
    : hasTexcoord?(texcoordType===2?'uvInput':'uvInput.xyz'):'vec3<f32>(0.0,0.0,1.0)';
 const uvExpression=textureDimension===3?cubeCoordinate:(texcoordType===0?'vec2<f32>(uvInput,0.0)':'uvInput.xy');
 const vertex=[
  transformSource,
  ...
  'result.uv='+(useTexture?uvExpression:(textureDimension===3?'vec3<f32>(0.0,0.0,1.0)':'vec2<f32>(0.0)'))+'; return result;','}'
 ].join('\n');
 const pixel=[io,useTexture?'@group(2) @binding(0) var image:'+(textureDimension===3?'texture_cube<f32>':'texture_2d<f32>')+'; @group(2) @binding(16) var imageSampler:sampler;':'',
 ...
 return {fixed:true,vertex:{wgsl:vertex,inputs:reflectedInputs,uniforms,constants:[],samplers:[]},pixel:{wgsl:pixel,uniforms:[],constants:[],samplers:useTexture?[{group:2,dimension:textureDimension,textureBinding:0,samplerBinding:16,sourceIndex:0}]:[]}};
}
