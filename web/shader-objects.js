...
export class ShaderObjects {
 ...
 pair(vertex,pixel){
  const vs=this.get(vertex,0),ps=this.get(pixel,1),key=vertex+':'+pixel;
  let cached=this.pairs.get(key);if(cached){this.pairs.delete(key);this.pairs.set(key,cached);return cached.pair;}
  const pair=this.translator.translatePair(vs.bytecode,ps.bytecode),bytes=JSON.stringify(pair).length*2;
