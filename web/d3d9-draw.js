...
export function decodeDraw(memory,pointer,length){
 ...
 if(magic!==0x39445245||![1,2,4].includes(kind)||count<1||count>1048576||count%({1:1,2:2,4:3})[kind]||declarationLength<16||declarationLength>520||declarationLength%8||stateCount!==20)throw RangeError('invalid draw header');
 ...
 const textures=take(16),samplers=Array.from({length:16},()=>take(14));
 if(p!==w.length)throw RangeError('trailing draw packet data');
 return{vertex,pixel,kind,count,first,index,base:base|0,max,streams,state,declaration,registers,textures,samplers};
}
