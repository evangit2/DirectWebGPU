...
const recentered={pointer:{cursorHidden:{horizontalSign:-1,verticalSign:-1},relativePointer:{horizontalSign:-1,verticalSign:-1}},touchJoystick:{cursorHidden:{horizontalSign:-1,verticalSign:-1},relativePointer:{horizontalSign:-1,verticalSign:-1}}};
...
assert.deepEqual(sent.at(-1),[4,410,300,0]);
...
assert.deepEqual(recenteredSent.at(-1),[4,407,308,0]);
assert.equal(cursors.at(-1).x,397);
canvasListeners.get('pointerup')({type:'pointerup',pointerId:5,clientX:80,clientY:80,buttons:0,button:0,preventDefault(){}});
assert.deepEqual(recenteredSent.at(-1),[3,407,308,1]);
document.pointerLockElement=null;
