...
async function start(long=false){
 ...
 try{
  ...
  worker.onmessage=({data})=>{
   ...
   const {type,...rest}=data;if(type==='draw-diagnostic'){report.drawDiagnostics??=[];if(report.drawDiagnostics.length<3)report.drawDiagnostics.push(rest.sample);return;}log(type,rest);
  ...
  gpuWorker.postMessage({type:'init',canvas:offscreen,port:channel.port1,drawDiagnostics:new URL(location.href).searchParams.has('drawDiagnostics')},[offscreen,channel.port1]);
