...
export class PipelineCache{
 ...
 async get(vertex,pixel,declaration,streams,state,{colorFormat='bgra8unorm',depthFormat='depth24plus-stencil8',topology='triangle-list',fixed=false,textured=false}={}){
  ...
  const pair=fixed?fixedFunctionPair(declaration,textured):this.objects.pair(vertex,pixel),layout=vertexLayout(declaration,pair.vertex.inputs,streams);
  ...
  const key=JSON.stringify([vertex,pixel,fixed,textured,layout,primitive,target,depthStencil,alpha]);
  ...
  const vertexSource=fixed?pair.vertex.wgsl:translator.vertexInputs(pair.vertex.wgsl,vertexWidths(layout));
