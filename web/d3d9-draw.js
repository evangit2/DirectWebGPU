...
export class DrawRenderer{
 ...
 async draw(packet){
  ...
  const entry=await this.cache.get(packet.vertex,packet.pixel,packet.declaration,packet.streams,packet.state,{topology,fixed:!!packet.fixed,textured:!!packet.textures[0],viewportSize:[width,height]});
