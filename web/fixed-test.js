...
export async function testFixed(device,renderer,buffers,textures,color,read,memory,texture){
 ...
 const old=new Uint32Array(diagnosticDrawPacket(0,0,vb,0,texture).buffer),decl=[0,0,0,0,2,0,0,0,0,0,12,0,3,0,10,0,0,0,28,0,1,0,5,0,255,0,0,0,17,0,0,0],at=11+48+old[10]*2;
 const words=[...old.slice(0,at),...new Uint32Array(new Uint8Array(decl).buffer),...old.slice(at+6+1312)];words[0]=0x33445246;words[9]=32;words[13]=36;
 ...
 words.push(...new Uint32Array(matrix.buffer),...new Array(96).fill(0));const payload=new Uint32Array(words);new Uint32Array(memory,4096,payload.length).set(payload);
 ...
 await renderer.draw(packet);renderer.flush();
 ...
 packet.state.set(15,1);packet.state.set(25,5);packet.state.set(24,255);await renderer.draw(packet);renderer.flush();
