...
const recentered={relativePointer:{horizontalSign:-1,verticalSign:-1},touchJoystick:{relativePointer:{horizontalSign:-1,verticalSign:-1}}};
...
assert.equal(directionalCode('ArrowUp',recentered,true,'desktop',true),'ArrowDown');
...
recenteredInput.warp(400,300);
assert.equal(cursors.at(-1).x,80);
...
assert.equal(cursors.at(-1).x,77);
document.pointerLockElement=null;
