...
export async function testDeviceBridge(){
 ...
 try{
  ...
  async function rpc(func,args){
   const buffer=new SharedArrayBuffer(16),words=new Int32Array(buffer);
   channel.port2.postMessage({func,args,buffer,retAddr:4});
   const wait=Atomics.waitAsync(words,1,0,10000);await wait.value;
   const result=Atomics.load(words,1)>>>0;if(!result)throw Error('GPU RPC timed out');if(error)throw Error(error);return result;
  }
  ...
  return{result:'passed',checks:['unsupported device configuration rejected','asynchronous GPU validation wakes blocked-style RPC','color/depth/stencil attachments allocated and cleared','GPU-to-GPU presentation validated','released handle rejected'],diagnosticPresents:1,HumusFrames:0,events};
