...
subprocess.run([str(r/'vendor/emsdk/upstream/emscripten/emcc'),*flags,*map(str,files),'-sMODULARIZE=1','-sEXPORT_ES6=1','-sENVIRONMENT=web,worker,node','-sALLOW_MEMORY_GROWTH=1','-sMAXIMUM_MEMORY=67108864','-sINITIAL_MEMORY=16777216','-sSTACK_SIZE=1048576','-sFILESYSTEM=0','-sEXPORTED_FUNCTIONS=["_malloc","_free","_shader_pair","_shader_reset","_shader_error","_shader_output","_shader_length"]','-sEXPORTED_RUNTIME_METHODS=["UTF8ToString","HEAPU8"]','-o',str(out/'mojoshader.js')],check=True)
...
for name in ['vertex','pixel','texture']:(out/(name+'.bin')).write_bytes(struct.pack('<'+'I'*len(f[name]),*f[name]))

import hashlib,datetime
artifacts=['mojoshader.js','mojoshader.wasm','shader_translation.js','shader_translation_bg.wasm','vertex.bin','pixel.bin','texture.bin']
