...
pub fn spirv_to_wgsl(bytes: &[u8]) -> Result<String, String> {
    ...
    let options=naga::front::spv::Options {adjust_coordinate_space:false, ..Default::default()};
    let split=combined_samplers::split(bytes)?;
    let module=naga::front::spv::parse_u8_slice(&split,&options).map_err(|e|format!("SPIR-V parse: {e}"))?;
    let info=naga::valid::Validator::new(naga::valid::ValidationFlags::all(),naga::valid::Capabilities::empty())
        .validate(&module).map_err(|e|format!("SPIR-V validation: {e:?}"))?;
    let wgsl=naga::back::wgsl::write_string(&module,&info,naga::back::wgsl::WriterFlags::empty())
        .map_err(|e|format!("WGSL emission: {e}"))?;
    let check=naga::front::wgsl::parse_str(&wgsl).map_err(|e|format!("WGSL parse: {}",e.emit_to_string(&wgsl)))?;
    naga::valid::Validator::new(naga::valid::ValidationFlags::all(),naga::valid::Capabilities::empty())
        .validate(&check).map_err(|e|format!("WGSL validation: {e:?}"))?;
    Ok(wgsl)
}
