...
const touchInverted={...inverted,touchJoystick:{relativePointer:{horizontalSign:-1,verticalSign:-1}}};
const recentered={keyboard:{relativePointer:{horizontalSign:-1,verticalSign:-1}},pointer:{relativePointer:{horizontalSign:-1,verticalSign:-1}},touchJoystick:{cursorHidden:{horizontalSign:-1,verticalSign:-1},relativePointer:{horizontalSign:-1,verticalSign:-1}}};
...
assert.equal(directionalCode('ArrowUp',touchInverted,true,'touchJoystick'),'ArrowUp');
assert.equal(directionalCode('ArrowLeft',touchInverted,false,'touchJoystick'),'ArrowLeft');
assert.equal(directionalCode('ArrowUp',touchInverted,true,'touchJoystick',true),'ArrowDown');
assert.equal(directionalCode('ArrowUp',recentered,true,'keyboard',true),'ArrowDown');
...
assert.deepEqual(sent.slice(-2),[[5,0x48,0x26,1],[6,0x48,0x26,1]]);
...
canvasListeners.get('pointerup')({type:'pointerup',pointerId:3,clientX:80,clientY:80,buttons:0,button:0,preventDefault(){}});
assert.equal(pointerLockRequests,0);
input.warp(400,300);
...
documentListeners.get('keydown')({type:'keydown',code:'ArrowUp',repeat:false,preventDefault(){}});
assert.deepEqual(recenteredSent.at(-1),[5,0x50,0x28,1]);
documentListeners.get('keyup')({type:'keyup',code:'ArrowUp',repeat:false,preventDefault(){}});
recenteredInput.setCursorVisible(false);
documentListeners.get('keydown')({type:'keydown',code:'ArrowUp',repeat:false,preventDefault(){}});
assert.deepEqual(recenteredSent.at(-1),[5,0x50,0x28,1]);
documentListeners.get('keyup')({type:'keyup',code:'ArrowUp',repeat:false,preventDefault(){}});
canvasListeners.get('pointerdown')({type:'pointerdown',pointerId:5,clientX:80,clientY:80,buttons:1,button:0,preventDefault(){}});
assert.equal(pointerLockRequests,1);
assert.deepEqual(recenteredSent.at(-1),[2,400,300,0x10001]);
document.pointerLockElement=canvas;
documentListeners.get('pointerlockchange')();
recenteredInput.warp(400,300);
assert.equal(cursors.at(-1).x,400);
recenteredInput.warp(410,310);
assert.equal(cursors.at(-1).x,410);
canvas.getBoundingClientRect=()=>({left:0,top:0,width:400,height:300});
const beforeSyntheticMove=recenteredSent.length;
canvasListeners.get('pointermove')({type:'pointermove',movementX:-320,movementY:-220,buttons:0,button:-1});
assert.equal(recenteredSent.length,beforeSyntheticMove);
canvasListeners.get('pointermove')({type:'pointermove',movementX:3,movementY:2,buttons:0,button:-1});
assert.deepEqual(recenteredSent.at(-1),[7,-3,-2,0]);
assert.equal(cursors.at(-1).x,407);
recenteredInput.warp(0,0);
canvasListeners.get('pointermove')({type:'pointermove',movementX:9,movementY:7,buttons:0,button:-1});
assert.deepEqual(recenteredSent.at(-1),[7,-9,-7,0]);
assert.equal(cursors.at(-1).x,0);
canvasListeners.get('pointerup')({type:'pointerup',pointerId:5,clientX:80,clientY:80,buttons:0,button:0,preventDefault(){}});
