...
let build,worker,gpuWorker,inputWorker,token,timer,probeWorker,inputBinding,HAS_BACKEND=false;
...
async function start(long=false){
 ...
 try{
  ...
  inputWorker.onmessage=worker.onmessage;inputWorker.onerror=worker.onerror;inputWorker.postMessage({type:'init',port:inputChannel.port1},[inputChannel.port1]);
